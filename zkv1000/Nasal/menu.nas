...
var menuClass = {
	...
	LIGHT: func {
		...
		if(b>0.5){
			setprop("/instrumentation/zkv1000/body-emission", 1);
		}else{
			setprop("/instrumentation/zkv1000/body-emission", 0);
		}
	},
	...
	NRST: func {
		...
		if (substr(title, 0, 8) == "NEAREST ") {
			...
			lockSearches();
            searchNearestNavaid("airport", 20);
	...
	enterNRST: func{
		...
		if (title == "NEAREST AIRPORTS") {
			...
			foreach (var r; keys(airport.runways)) {
				...
				if(getprop("/sim/version/flightgear")=="2.6.0"){
					...
					if(runway.ils_frequency_mhz!=nil){
							ligne = sprintf("%s ILS %3.2f",ligne,runway.ils_frequency_mhz);
							id = "ils";
							freq = runway.ils_frequency_mhz;
					}
	...
	D: func {
		...
		if(substr(title, 0, 9) == "DIRECT TO"){
			...
			DLINE = [];
			D_ID_CHAR = searchLastChar(D_ID,size(D_ID)-1);
			
			me.setSmallFMSknobAction("selectD_letterIncrease");
			me.setLargeFMSknobAction("selectD_letterChange");
			me.setENTsoftkeyAction("D_ENT_letter");
			me.setCLRsoftkeyAction("D_CLR_letter");
			me.D_display();
			me.mud.highlight();
		}
	},
	...
	selectD_letterIncrease: func(dir){
		D_ID_CHAR = D_ID_CHAR + dir;
		if(D_ID_CHAR<0){
			D_ID_CHAR = 35;
		}
		if(D_ID_CHAR>35){
		...
		D_ID = substr(D_ID,0,size(D_ID)-1) ~ tab_chiffres_lettres[D_ID_CHAR];
		
		me.D_display();
	},
	
	selectD_letterChange: func(dir){
		if(dir==1){
			D_ID = D_ID ~ tab_chiffres_lettres[D_ID_CHAR];
		}elsif(dir==-1){
			if(size(D_ID)>1){
				D_ID = substr(D_ID,0,size(D_ID)-1);
				D_ID_CHAR = searchLastChar(D_ID,size(D_ID)-1);
			}
		}
	...
	D_display: func{
		...
		me.mud.add(["ID = "~D_ID], void);
		me.mud.add(["------------------------------------------"], void);
		
		##recherche gps globale
		var plots = [];
		DLINE = [];
		props.globals.getNode("/instrumentation/gps/scratch/max-results", 1).setIntValue(max_result);
		...
		props.globals.getNode("/instrumentation/gps/scratch/query",1).setValue(D_ID);
		...
		while (flag_continue and i<max_result) {
			...
			var plot = { id: id,distance: range,bearing: bearing,type: type};
			append(plots,plot);
		...
		if(nb_plots>max_result){
			nb_plots = max_result;
		}
		for(var j=0;j<nb_plots;j=j+1){
			append(DLINE,plots_sorted[j].id ~ "," ~ plots_sorted[j].type);
			me.mud.add([sprintf("%s DST %.1fNM BRG %03i %s",plots_sorted[j].id,plots_sorted[j].distance,plots_sorted[j].bearing,plots_sorted[j].type)], void);
		}

		me.mud.open();
	},
	
	selectD_select: func(dir){
		if(dir==-1){
			if(me.mud.getSelectedBlock()>2){
				me.mud.select(me.mud, dir);
			}else{
				me.mud.highlight(0);
				me.mud.highlight(2);
	...
	selectD_select10: func(dir){
		if(dir==-1){
			if(me.mud.getSelectedBlock()>20){
	...
	D_ENT_letter: func{
		if(size(DLINE)>0){
			me.setSmallFMSknobAction("selectD_select");
			me.setLargeFMSknobAction("selectD_select10");
			me.setENTsoftkeyAction("D_ENT_select");
			me.setCLRsoftkeyAction("D_CLR_select");
			me.mud.highlight(2);
		}
	},
	
	D_ENT_select: func{
		var prec_autopilot_alt = getprop("/autopilot/settings/target-altitude-ft");
		lockSearches();
		var id = split(",",DLINE[me.mud.getSelectedBlock()-2])[0];
		var type = split(",",DLINE[me.mud.getSelectedBlock()-2])[1];
		...
		setprop("/instrumentation/gps/selected-course-deg",getprop("/instrumentation/gps/wp/wp[1]/bearing-mag-deg"));
		me.closeMud();
		me.setSmallFMSknobAction("");
		me.setLargeFMSknobAction("");
		me.setENTsoftkeyAction("");
		me.setCLRsoftkeyAction("");
		setprop("/autopilot/settings/target-altitude-ft",prec_autopilot_alt);
	},
